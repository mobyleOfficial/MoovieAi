...
- Do NOT dispatch the `flutter-validator` subagent (or any other pipeline agent) from inside `reviewer`. The `flutter-validator → reviewer` direction is the only sanctioned dispatch — dispatching back creates a cycle and undermines the read-only separation. If you find yourself wanting flutter-validator's checklist coverage, call the three sub-reviewers directly (already part of Step 3); they are the shared analysis layer.
