...
while iter < max_iter:
    ...
    # CRITICAL: gh_unresolved_threads filters to agent-authored threads only. Threads from human
    # reviewers or external bots (gemini-code-assist, etc.) must NOT be auto-judged/resolved by
    # this agent — they're surfaced via the defer path so a human handles them.
    open_threads = gh_unresolved_threads(pr, agent_login_only=True)

    # Step F — per thread: fix-or-reject (autonomous)
    # severity and confidence come from FINDING_META (set in Step D when we posted the comment):
    # FINDING_META is a bash assoc array keyed by databaseId, value is "<severity>|<confidence>".
    # Lookup: meta="${FINDING_META[$thread_db_id]:-MEDIUM|1.0}"; severity="${meta%|*}"; confidence="${meta#*|}".
    # Default fallback MEDIUM|1.0 covers external-author edge cases (shouldn't fire — gh_unresolved_threads filters those out — but is a safe degenerate value).
    for thread in open_threads:
        thread_db_id = thread.comments.nodes[0].databaseId
        meta = FINDING_META.get(thread_db_id, "MEDIUM|1.0")
        severity, confidence = meta.split("|")
...
|---|---|
| `gh_root_comments(pr)` | `gh api --paginate "repos/${REPO}/pulls/$pr/comments" --jq '[.[] \| select(.in_reply_to_id == null) \| {id, node_id, path, line, original_line, body, user: .user.login}]'`. **MUST include `original_line`** — comments posted via the reviews API have `line: null` in the REST response (the value lives in `original_line`); a downstream `dedupe` using only `line` would never match prior comments and would re-post every finding on every pass. |
| `gh_unresolved_threads(pr, agent_login_only=True)` | Paginated GraphQL query that enumerates open review threads on the PR. **Author filter (CRITICAL):** when `agent_login_only=True` (always the default for caller use), the returned set EXCLUDES threads whose root-comment author is anyone other than ultimate-developer's own GitHub login. Resolve agent's login once at startup: `AGENT_LOGIN=$(gh api user --jq .login)`. Then in the GraphQL/jq filter, also require `select(.comments.nodes[0].author.login == $AGENT_LOGIN)`. **Why:** Step F autonomously decides fix/reject/resolve on returned threads — applied to a human reviewer's `REQUEST_CHANGES` thread or a gemini-code-assist comment, the agent would silently dismiss human feedback. Excluding external-author threads from this enumeration means Step F never sees them and they remain open for human handling (caller may surface them via a separate "external threads pending" advisory in the summary report). **Distinct from `find_thread`**: `find_thread` looks up ONE specific thread by `comment_id` (for resolving a known thread by ID — used by reply_thread/resolve_thread plumbing, no author check needed since the caller already chose the thread); `gh_unresolved_threads` enumerates ALL agent-authored open threads for Step F's auto-judge loop. Implementation: same outer `while :; do` cursor-pagination loop as `find_thread` but (a) no `comment_id` filter and (b) the author-filter jq selector above. Pseudocode: `cursor=""; result=[]`; loop: build `args` array with `first:100,after:$cursor` (omit `after` on first call — same "no literal null" rule as `find_thread`); call `gh api graphql` with the `reviewThreads` query; append `nodes \| map(select(.isResolved == false and .comments.nodes[0].author.login == "$AGENT_LOGIN"))` to `result`; advance cursor or break. Return `result` as a JSON array of `{id, isResolved, comments: {nodes: [{databaseId, body, author: {login}}]}}`. |
...
| `post_inline_review(pr, findings, pass)` | The `jq`-built reviews-API POST documented in `agents/reviewer.md` Step 9. Use `event: "COMMENT"` (advisory, never gate). Top-level body matches the "ultimate-developer review pass `<N>`" template below. **Populates `FINDING_META`** — after the POST returns successfully, parse the response's `comments` array (each entry has the `id` GitHub assigned to the comment) and populate the bash assoc array `FINDING_META` (`declare -A FINDING_META` at agent start). **Normalize across reviewer / validator / architect-review schemas** — only `reviewer` emits `risk` + `confidence`; `validator` and `architect-review` emit `severity` (different enums, no confidence). Build the entry as `FINDING_META[$comment_id]="${finding.risk:-${finding.severity:-MEDIUM}}|${finding.confidence:-1.0}"` so non-reviewer findings get sensible defaults that still hit a `judge_finding` decision-table branch. Step F looks up severity + confidence from this map keyed by the comment's databaseId. Without normalization, validator/architect findings would store `|` or `null|null`, Step F would read `severity=""`, and the `judge_finding` decision table would silently match no branch — the loop becomes unable to act on those findings. Mirror the map entries to `"$REVIEW_LOG"` as `finding_meta[$id]=<sev>|<conf>` so re-entry recovery can restore the map after interruption. |
...
Each git repository (meta-repo + each submodule) has its OWN stash stack — `git stash list` in the meta-repo only shows meta-repo stashes; submodule stashes are invisible from there. Stashes from `apply_fix` during impl phase live in the submodule. The scan must run in every repo we might have touched.

**At agent start** (after Kickoff Step 1, before entering any phase loop), scan the meta-repo:
...
check_orphan_stashes_in_cwd() {
  ...
  if [ "${#orphan_stashes[@]}" -gt 0 ]; then
    ...
    ORPHAN_DETAIL="$repo_label: $(git stash list | grep ud-apply_fix-pre)"
    escalate "" "orphan ud-apply_fix-pre stash(es) found from prior interrupted run in $repo_label" "detail=${ORPHAN_DETAIL}"
    exit 2
  fi
}

# Always check meta-repo at startup.
check_orphan_stashes_in_cwd "meta-repo"
```

**Additionally, at every `cd "$REPO_DIR"` in the impl-phase per-repo loop setup** (before dispatching the implementer for that submodule), re-run the scan in the submodule's CWD:

```bash
cd "$REPO_DIR"
check_orphan_stashes_in_cwd "$REPO_DIR"  # catches submodule-local orphans from a prior killed impl run
```

This must happen BEFORE the implementer is dispatched — otherwise a `git stash push` from this run would layer atop an orphan from the last run, hiding it.
