...
3. **flutter-implementer-tester** — Implement approved spec in the target Flutter submodule with tests
4. **flutter-validator** — Read-only validation against spec + ecosystem rules → report in `research/reviews/<feature>-code-review.md`. Blocks merge on critical issues.

Each agent has specific tool allowlists and responsibilities defined in its metadata. All agents understand the MoovieAi ecosystem structure and can work across the project's Flutter submodule(s) and optional backend.
