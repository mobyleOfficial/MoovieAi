...
const tools = {
    "sync-submodule": async (args) => {
        ...
        try {
            ...
            const commit = execSync(`cd ${name} && git rev-parse --short HEAD`, {
                cwd: ROOT_DIR,
                encoding: "utf-8",
            }).trim();
            ...
            throw new Error(`Failed to sync submodule '${name}': ${error.message}`);
        }
    },
    ...
    "create-release-branch": async (args) => {
        ...
        try {
            ...
            throw new Error(`Failed to create release branch: ${error.message}`);
        }
    },
    "open-pull-request": async (args) => {
        ...
        try {
            ...
            throw new Error(`Failed to open PR: ${error.message}`);
        }
    },
    "check-status": async () => {
        try {
            const moovieStatus = execSync("git -C moovie rev-parse HEAD && git -C moovie rev-parse origin/main", {
                cwd: ROOT_DIR,
                encoding: "utf-8",
            });
            const backendStatus = execSync("git -C backend rev-parse HEAD && git -C backend rev-parse origin/main", {
                cwd: ROOT_DIR,
                encoding: "utf-8",
            });
...
const toolDefinitions = [
    {
        name: "sync-submodule",
        description: "Sync a submodule to the latest remote commit and update the reference",
        inputSchema: {
            type: "object",
            properties: {
                name: {
    ...
    },
    {
        ...
        description: "Sync submodules, checkout develop, and create a feature branch",
        inputSchema: {
            type: "object",
            properties: {
                name: {
    ...
    },
    {
        ...
        description: "Sync submodules, checkout develop, and create a release branch",
        inputSchema: {
            type: "object",
            properties: {
                version: {
    ...
    },
    {
        ...
        description: "Create a pull request (feature/fix branches → develop, release branches → main)",
        inputSchema: {
            type: "object",
            properties: {
                title: {
