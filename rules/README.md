...
These rules apply when working inside the `moovie/` submodule. They are enforced by the `flutter-implementer-tester` agent, whose `Stop` hook chain runs `flutter analyze` + `flutter test` + structural validators. Hook scripts live in `.claude/hooks/` but are not project-wide.
